...
$('.tabClick').on('click', function() {
    ...
    if (proceed) {
        search_browse_itineraries(serviceId, privateType, groupType, familyType, cityid, date, themesid, hostType, itineraryLang, girdClass);
    }
    if (serviceId == 1) {
        serviceName = 'walk';
    ...
    history.pushState({
        isMine: true
    }, 'home |User', '<?php echo base_url();?>' + 'itineraries/browse_itineraries/' + serviceName);

});
  
function search_browse_itineraries(serviceId, privateType, groupType, familyType, cityid, date, themesid, hostType, itineraryLang, girdClass) {
    $.ajax({
        type: 'post',
