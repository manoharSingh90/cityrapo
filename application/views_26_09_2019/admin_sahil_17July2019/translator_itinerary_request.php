      ...
      <div class="boxCover">
        <div class="cform-box">
          <ul class="nav nav-tabs cform-tab" role="tablist">
            <li class="nav-item"> <a class="nav-link " href="<?php echo base_url();?>translator_all_itineraries">All Itineraries</a> </li>
            <li class="nav-item"> <a class="nav-link active" href="<?php echo base_url();?>translator_reqitineraries">Requests 
				<?php if($notifyData>0){?>
			 <b class="badge">
        ...
        <table id="tableStyle" class="display" style="width:100%">
          ...
          <tbody id="itineries_append_data">		  
              <?php $this->load->view('admin/translator_itinerary_element');?>          
          </tbody>
        </table>
      </div>
    </div>
...
function reloadSearch(){
	  ...
	  var proceed =true;	  
	   if(proceed){
		    $.ajax({
				  type:'post',
				  url:'<?php echo base_url()?>admin/translator_reqitineraries',
