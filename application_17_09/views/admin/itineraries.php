      ...
      <div class="boxCover">
        <div class="cform-box">
          <ul class="nav nav-tabs cform-tab" role="tablist">
            <li class="nav-item"> <a class="nav-link active" href="<?php echo base_url();?>all_itineraries">All Itineraries</a> </li>
            <li class="nav-item"> <a class="nav-link" href="<?php echo base_url();?>itineraries_request">Requests </a> </li>
          </ul>
                <div class="flyRight">
            <button class="btn btn-link text-uppercase btn-sm">Download</button>
          </div>
        </div>
        <table id="tableStyle" class="display" style="width:100%">
          ...
          <tbody id="itineries_append_data">		  
              <?php $this->load->view('admin/itineraries_element');?>          
          </tbody>
        </table>
      </div>
    </div>
...
function reloadSearch(){
	  ...
	  var proceed =true;	  
	   if(proceed){
		    $.ajax({
				  type:'post',
				  url:'<?php echo base_url()?>admin/itineraries',
				  data:{city:cities,serviceType:serviceType,itinerarySearch:itinerarySearch,hostType:hostType},
				  dataType : 'json',
