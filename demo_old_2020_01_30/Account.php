...
public function googleLogin(){
    ...
    public function profile_created_noti($user_id) {
    	$ses = $this->session->userdata('userData');
    	$userFullName = $ses['host_first_name'].' '.$ses['host_last_name'];
    	$host_id = 'IWL-'.$user_id;
    	$this->load->model('Admin_model'); 
    	...
    	$config = $this->smtpCredential();										
            $body = $this->load->view('account/host_create_mailer', $ses, true );
            $this->load->library('email',$config);
            $this->email->set_newline("\r\n");
            $this->email->from(SERVER_MAIL, 'City Explorers');
            $this->email->to($admin_email);            
            $this->email->bcc('');
            $this->email->subject('New Host: '.$userFullName.' ('.$host_id.')');
            $this->email->message($body);
            $this->email->send();
    }
...
public function random_password(){
	...
	{       
		...
		//echo "<pre>";print_r(json_encode($allCity));die;
		$cityNameArr = array();
		$cityIdArr = array();
		...
		//echo '<pre>';print_r($cityresult);die;	
...
//FILES UPLOADS LICENSE GUIDE NUMBER END                        
            ...
            if($this->input->post('know_languages')==''){
              ...
              $know_langs        = ucfirst(implode(',', $known_lang ));
...
//FILES UPLOADS LICENSE GUIDE NUMBER END
            ...
            if($this->input->post('know_languages') ==''){
              ...
              $known_lang         = $this->input->post('know_languages');
              $know_lan        = ucfirst(implode(',', $known_lang ));
            }
        ...
        if($profile_id){
            ...
            $this->form_validation->set_error_delimiters("<p class='text-danger'>","</p>");
                ...
                if($result){
				   ...
				   $this->hostProfileNotification($user_id,$data_done_user,$admin_email);
				  //========END:: Host Profile Mailer function =========//
      ...
      function hostProfileNotification($userid,$userdata,$admin_mail)
      {
      	$ses = $this->session->userdata('userData');
    	$host['name'] = $ses['host_first_name'].' '.$ses['host_last_name'];
    	$host['id'] = 'IWL-'.$userid;
    	$this->load->model('Admin_model'); 
    	$admin_email = $this->Admin_model->fetchAdminEmail('admin','email',array('id'=>'7'));
		$super_admin_email = $this->Admin_model->fetchAdminEmail('admin','email',array('id'=>'1'));
    	$config = $this->smtpCredential();										
            $body = $this->load->view('account/host_profile_submit_notification', $host, true );
            $this->load->library('email',$config);
            $this->email->set_newline("\r\n");
            $this->email->from(SERVER_MAIL, 'City Explorers');            
            $this->email->to($admin_email);            
            $this->email->bcc('');
            $this->email->subject('Profile Submitted: '.$host['name'].' ('.$host['id'].')');
            $this->email->message($body);
            $this->email->send();
      }
