...
class Home extends CI_Controller {
	...
	public function index(){	
		...
		/*   Change By Manohar  For All Language on 13 Sep 2019	*/

		// $itineraryLang = $this->Itinerarie_model->fetchItineraryLanguages();
		 $itineraryLang = $this->Itinerarie_model->fetchAllLanguages();
...
/*public function madhya_pradesh() {
	$logedin  = $this->session->userdata('id');
	//$cityData = $this->Itinerarie_model->getcities();
	$cityData = $this->Itinerarie_model->getMpCities();
	$themesData = $this->Itinerarie_model->selectThemes();	
	$hostimage = $this->Itinerarie_model->getProfileimage($logedin);
	$allowItinerary = $this->Itinerarie_model->allowHost($logedin);
	$hostTypeData = $this->Admin_model->fetchHostType();
	///======== Change by Manohar  on 13 sep 2019 	=========//
	//$itineraryLang = $this->Itinerarie_model->fetchItineraryLanguages();
	$itineraryLang = $this->Itinerarie_model->fetchAllLanguages();
	 
	$serviceId = $this->input->post('serviceId')==''?1:$this->input->post('serviceId');
	...
	$itineraryData = $this->Itinerarie_model->loadSearchMpItineraries($serviceId,$privateType,$groupType,$cityid,$date,$themesid,$familyType,$hostType,$itinerary_Lang,$page);
	if(!empty($itineraryData)){
	   $data['iterator'] = $itineraryData;
	}else{
	  $data['iterator'] = '';
	}		
	//echo"<pre>";print_r($data['iterator']);die;
	 $this->load->helper('getallthemes');
	 $this->load->helper('getcategory');
	 $this->load->helper('hostname');
	 $this->load->helper('getfamilydata');
	...
	if($this->input->is_ajax_request())		
		{			
		 ...
		 $this->load->view('home/madhya_pradesh',compact('data','serviceId','cityData','themesData','hostimage','allowItinerary','hostTypeData',       'itineraryLang'));
		}	
}*/
...
public function moreAboutUsForm() {
	...
	if($formData['mfu_email']!='') {
		$config = $this->smtpCredential();
		$adminMail['Title'] = ucwords($formData['mfu_title']);
		$adminMail['Fname'] = ucwords($formData['mfu_first_name']);
		$adminMail['Lname'] = ucwords($formData['mfu_last_name']);
		$adminMail['email'] = $formData['mfu_email'];
		$adminMail['phone_no'] = $formData['mfu_phone'];
		$adminMail['interest'] = $formData['mfu_interest'];
		$adminMail['travel_plan'] = $formData['mfu_travel_plan'];
		$adminMail['send_news'] = isset($formData['mfu_agree'])?strtoupper($formData['mfu_agree']):'NO';
		
	   $body = $this->load->view('mailer/more_from_us', $adminMail, true );
				$this->load->library('email',$config);
				$this->email->set_newline("\r\n");
				$this->email->from('help@cityexplorers.in', 'City Explorers');
				$this->email->to($admin_email);
        		$this->email->cc($super_admin_email);
				$this->email->subject('More From Us: '.ucwords($formData['mfu_title']));
				$this->email->message($body);
				$this->email->send();
			echo 'success';die;	
	}
...
public function detail_itineraries()
	{
	  ...
	  //echo $serviceId;die;
	   ...
	   $this->load->view('home/detail_itineraries',compact('itineraryData','itinerary_id','serviceId',
	                                                        'userType','userLang','backStatus','hostimage'));
	}
...
public function detail_itineraries_sessions(){
	   ...
	   $this->load->view('home/detail_itineraries_sessions',compact('itineraryData','itinerary_id','serviceId',
	                                   'userType','userLang','allSpeakers','attendeesData','backStatus','hostimage'));
}	
...
public function detail_itineraries_experiences(){
	   ...
	   $this->load->view('home/detail_itineraries_experiences',compact('itineraryData','itinerary_id','serviceId',
	                                                                  'userType','userLang','allSpeakers','backStatus','hostimage'));
}	
...
public function detail_itineraries_meetup(){
	   ...
	   $this->load->view('home/detail_itineraries_meetup',compact('itineraryData','itinerary_id','serviceId','userType',
	                                                              'userLang','allSpeakers','attendeesData','backStatus','hostimage'));
}	
