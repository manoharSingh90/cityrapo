...
public function work_with_us(){
 ...
 public function pledgeFormSubmit() {
 	$loggedInUser = $this->session->userdata('id');
 	...
 	if($dataForm['pledge_email']!=''){	  
		   $config = Array(
				...
				$adminMail['userName'] = $dataForm['pledge_name'];
				$adminMail['userEmail'] = $dataForm['pledge_email'];
				$adminMail['userMobile'] = $dataForm['pledge_phone'];
				$adminMail['userDate'] = $dataForm['pledge_date'];
				$adminMail['userLanguage'] = $dataForm['language'];
				if(!empty($loggedInUser))
					$adminMail['userId'] = $loggedInUser;
				//echo '<pre>';print_r($adminMail);die;
				$loginId = isset($loggedInUser)?' (IWL-'.$loggedInUser.')':' ';
				$body = $this->load->view('mailer/pledge_mail', $adminMail, true );
				$this->load->library('email',$config);
				$this->email->set_newline("\r\n");
				$this->email->from($dataForm['pledge_email'], 'City Explorers');
				//$this->email->to($admin_email,$super_admin_email, 'City Explorers');				
				$this->email->to(array($admin_email,$super_admin_email), 'City Explorers');				
				$this->email->subject('Pledge taken by: '.$dataForm['pledge_name']. $loginId);
