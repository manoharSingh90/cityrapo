  ...
  <ul>
    ...
    <div class="profilePage-info">
      <form id="formItinerary" method="post" enctype="multipart/form-data">
		...
		<?php 
			if($selectOtherLang!=='English' && $selectOtherLang!=='ENGLISH' && $selectOtherLang!='english'){
			?>
			   ...
			   <?php 
                ...
                <li>
                  <label class="tagBox <?php echo @$indoorclass;?>">
                    <input type="checkbox" class="ignore" name="meetup_itinerary_type[]" value="Indoor" <?php if(isset($meetupMerge['Indoor'])=='Indoor') echo 'checked="checked"'; ?> data-rule-required="false"/>
                    Indoor</label>
                </li>
                <li>
...
$(document).on('click', '.clearSponsor', function(e) {
    ...
    $("#addSponsor").parent().show();
        ...
        $('[name="type_category"], [name="origin_otherCity"], [name="itinerary_other_title"], [name="other_tag_line"], [name="other_itinerary_description"]').addClass('ignore').removeAttr('required').data("rule-required", false);
						...
						validator.destroy();
						$('#formItinerary').attr('action','adminDoneMeetupItinerary');
						$('.loadingWrap').show();
						$("#formItinerary").validate({
										errorElement: 'small',
										errorPlacement: function(error, element) {
