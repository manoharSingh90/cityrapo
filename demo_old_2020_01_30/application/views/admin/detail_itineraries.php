...
-->
	  ...
	  <span data-toggle="modal" data-target="#rejectModal">Reject</span></a>
	   ...
	   <!-- <?php if($itineraryData->translator==0){
		...
		<?php if($itineraryData->translator==0){
		  	$targetId = '#approveModal';
		  }else if($itineraryData->translator==1 && $itineraryData->translator_confirm==0 && $itineraryData->translator_type==0) {
		  	$targetId = '#approveModal_translator';
		  }else if($itineraryData->translator_confirm==0 && $itineraryData->translator_type!=0) {
		  	$targetId = '#approveModal_translator';
		  }else{
		  	$targetId = '#approveModal';
		  }
...
<!-- Translator ALERT MODAL -->
<div class="modal fade" id="sendModal_translator" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-body">
        <div class="confirmationText">
          <p class="text-center">The itinerary has been sent to translator successfully </p>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-link text-primary" data-dismiss="modal">Close</button>
 ...
 if(proceed){
	  $.ajax({
			...
			success:function(data){
				$('.loadingWrap').hide();
				$('#sendModal_translator').modal('show');
				 console.log(data);
				 if($.trim(data)=='success'){
					 location.reload();
					 }
				//setTimeout(function(){ $('#sendModal_translator').modal('show'); }, 4000);	
				}
	   });
	 }	  
