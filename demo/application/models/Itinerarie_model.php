...
class Itinerarie_model extends CI_Model
{
 ...
 public function updateDraftData($userId,$itinerarySaveArr){
	  ...
	  'sponsors_img'=>$itinerarySaveArr['sponsors_img'],'video'=>$itinerarySaveArr['video'],'youtube_video'=>$itinerarySaveArr['youtube_video'],
...
public function deleteStopsdata($userId,$itineraryid){
 ...
 public function getItineraryLastStatus($lastId){
  ...
  public function updateDoneData($userId,$itinerarySaveArr){
	  ...
	  'additional_img_1'=>$itinerarySaveArr['additional_img_1'],'additional_img_2'=>$itinerarySaveArr['additional_img_2'],
	  'additional_img_3'=>$itinerarySaveArr['additional_img_3'],'feature_img'=>$itinerarySaveArr['feature_img'],
	  'video'=>$itinerarySaveArr['video'],'youtube_video'=>$itinerarySaveArr['youtube_video'],'status'=>$itinerarySaveArr['status'],'login_status'=>$itinerarySaveArr['login_status'],
...
public function getItinearyStatus($id,$user_id) {
	$data =  $this->db->select('id,status,admin_status')
				->from('creates_itinerary')
				->where('id',$id)
...
public function updateMeetupDraftData($userId,$itinerarySaveArr){
	  ...
	  'login_status'=>$itinerarySaveArr['login_status'],'video'=>$itinerarySaveArr['video'],'youtube_video'=>$itinerarySaveArr['youtube_video'],
	  'translator'=>$itinerarySaveArr['translator'],'term_condition'=>$itinerarySaveArr['term_condition'],'frequency_type'=>$itinerarySaveArr['frequency_type'],'frequency_off_days'=>$itinerarySaveArr['frequency_off_days']);
...
public function editUpdateMeetupDoneData($userId,$itinerarySaveArr,$itinerary_id){
	  ...
	  'week4_days'=>$itinerarySaveArr['week4_days'],'week5_days'=>$itinerarySaveArr['week5_days'],
	  'additional_img_1'=>$itinerarySaveArr['additional_img_1'],'additional_img_2'=>$itinerarySaveArr['additional_img_2'],
	  'additional_img_3'=>$itinerarySaveArr['additional_img_3'],'feature_img'=>$itinerarySaveArr['feature_img'],'sponsors_img'=>$itinerarySaveArr['sponsors_img'],'video'=>$itinerarySaveArr['video'],'youtube_video'=>$itinerarySaveArr['youtube_video'],'status'=>$itinerarySaveArr['status'],'login_status'=>$itinerarySaveArr['login_status'],'admin_status'=>$itinerarySaveArr['admin_status'],
...
function countTotalItinerari($serviceId,$privateType,$groupType,$cityName,$date,$themesid,$familyType,$hostType,$itineraryLang) 
{
	$condition = "creates_itinerary.admin_status = 5  and creates_itinerary.service_id = '$serviceId'";
	...
	$query = $this->db->query("select creates_itinerary.*,'users_profile.user_id,users_profile.host_verification_type' from creates_itinerary 
		           JOIN users_profile ON users_profile.user_id = creates_itinerary.user_id where $condition");
		//echo $query->num_rows();
...
//=========== Load more Itinerary data Madhya Pradesh ============//
public function loadSearchMpItineraries($serviceId,$privateType,$groupType,$cityName,$date,$themesid,$familyType,
	                                   $hostType,$itineraryLang,$page){
	    $offset = 4*$page;
        $limit = 4;
		$condition = "creates_itinerary.admin_status = 5  and creates_itinerary.service_id = '$serviceId'";
...
public function fetchAllLanguages(){
	$data = $this->db->select('iwl_languages.language_name as itinerary_language')
	                 ->from('iwl_languages')
					// ->where('admin_status',5)
